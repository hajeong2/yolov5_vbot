        ...
        if start_idx != -1:
            ...
            if end_idx != -1:
                ...
                if new_cracks > 0:  # 진짜 새 균열이 있을 때만!
                    ...
                    # 전송용 이미지 준비
                    try:
                        image_b64 = shrink_for_send(img)
                        
                        # payload 생성
                        payload = {
                            "ts": int(current_time * 1000),
                            "crack_count": stats["unique_total"],   # Node-RED 호환용 (누적 유니크)
                            "new_cracks": new_cracks,              # 이번에 새로 발견된 균열
                            "unique_total": stats["unique_total"], # 누적 유니크 균열 (상세정보)
                            "frame_detections": frame_crack_count, # 이 프레임 박스 수 (참고용)
                            "image_b64": image_b64
                        }
